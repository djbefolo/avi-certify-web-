...
export async function GET(request: NextRequest) {
  ...
  try {
    const summary = await processDueProfileReminders();
    return jsonResponse({ ok: true, ...summary }, 200);
  } catch (error) {
    console.error("[profile-reminder] Cron failed", {
      code:
        error && typeof error === "object" && "code" in error
          ? String(error.code)
          : "UNEXPECTED_ERROR",
    });
    return jsonResponse({ error: "Le traitement des relances a echoue." }, 500);
  }
}
